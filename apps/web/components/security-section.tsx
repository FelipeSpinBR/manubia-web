  ...
  return (
    <section className="section-space">
      <div className="container-site grid gap-8 lg:grid-cols-[1.2fr_1fr]">
        <div>
          <h2 className="text-3xl font-bold text-[#1f2d24]">Sua informação financeira protegida com seriedade</h2>
          <p className="mt-4 max-w-3xl text-lg text-[#5a6b61]">
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            {pillars.map((pillar) => (
              <article key={pillar.title} className="card-soft p-5">
                <h3 className="font-semibold text-[#1f2d24]">{pillar.title}</h3>
                <p className="mt-2 text-sm text-[#5a6b61]">{pillar.text}</p>
              </article>
            ))}
          </div>
        </div>

        <div className="card-soft min-h-72 bg-gradient-to-br from-[#edf7f0] to-[#dff3e6] p-6" />
      </div>
    </section>
  );
}
